...
export default function Web3ContextProvider({
  ...
  const connectWallet = async (provider: ProviderLabel) => {
    ...
    console.log("injected wallets ", injectedWallets);
    const walletStatus = (injectedWallets as WalletModule[]).find((ele: WalletModule) => ele.label === provider);
    ...
    if (connectWallets[0]) {
      ...
      localStorage.setItem(
        "connectedWallet",
        JSON.stringify({ accounts, chains, label }),
      );
